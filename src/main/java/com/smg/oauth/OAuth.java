...
import com.smg.oauth.config.OauthOptions;
...
public class OAuth {
	...
	public String redirect( String provider, String url ) {
		String location = "";
		try {
			String urlToRedirect = properties.getProtocol() + "://" + properties.getHost() + ( !properties.getPort().isEmpty()
			                                                                                   ? ":" + properties.getPort()
			                                                                                   : "" ) + url;
			...
			location = this.injector.getConfig().getOauthUrl() +
			           this.injector.getConfig().getOauthdBase() + "/" + provider + "?k=" + this.injector.getConfig()
			                                                                                             .getAppKey() + "&opts=" + opts + "&redirect_type=server&redirect_uri=" + urlToRedirect;
	...
	public JSONObject refreshCredentials( JSONObject credentials, boolean force ) {
		try {
			...
			if ( ( credentials.has( "refresh_token" ) && ( credentials.has( "expires" ) && date.getTime() > ( ( Date ) credentials.get( "expires" ) )
					                                                                                                .getTime() ) ) || force ) {
				HttpWrapper request = this.injector.getRequest();

				String url = this.injector.getConfig().getOauthUrl() + this.injector.getConfig().getOauthdBase() + "/refresh_token/" + credentials
						                                                                                                                       .get( "provider" );
